/**
 * On-Demand Test Data Step Definitions
 * 
 * These steps create PERSISTENT test data (no cleanup) for manual testing.
 * Use with @onDemand tag to run manually when sample data is needed.
 * 
 * ════════════════════════════════════════════════════════════════════════════
 * 🔒 LOCKED VERSION - STABLE & TESTED
 * ════════════════════════════════════════════════════════════════════════════
 * 
 * This code has been tested and verified to work correctly for:
 * - Creating 15 diverse Accounts from Excel data (all Account Types covered)
 * - Creating 10 diverse Leads from Excel data (all regions covered)
 * - Creating 10 diverse Contacts from Excel data (all regions covered)
 * - Creating 10 diverse Opportunities from Excel data (diverse Account Types)
 * 
 * DO NOT MODIFY without:
 * 1. Understanding the full impact on existing test data creation
 * 2. Testing all scenarios (Accounts, Leads, Contacts, Opportunities)
 * 3. Verifying field validation (removed invalid fields: BirthDate, JobTitle → Title, Jigsaw length)
 * 4. Ensuring regional diversity and Account Type diversity are maintained
 * 
 * Last verified: 2025-12-05
 * Status: ✅ WORKING - All test data creation scenarios passing
 * ════════════════════════════════════════════════════════════════════════════
 */
...
When('I create 10 real-world Leads from Excel data', async function (this: AutomationWorld) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (!leadEmail || leadEmail === 'NULL' || !leadEmail.includes('@')) {
      ...
      leadEmail = `qa.${firstName.toLowerCase()}.${lastName.toLowerCase()}@${domain}`;
    } else {
      leadEmail = `qa.${leadEmail}`;
    }
    ...
    const uniqueEmail = leadEmail.includes('@') 
      ...
      : `qa.${firstName.toLowerCase()}.${lastName.toLowerCase()}${uniqueSuffix}@${companyName.toLowerCase().replace(/\s+/g, '')}.com`;
...
When('I create 10 real-world Contacts from Excel data', async function (this: AutomationWorld) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (!contactEmail || contactEmail === 'NULL' || !contactEmail.includes('@')) {
      const accountName = `QA Parent Account ${i + 1}`;
      const domain = accountName.toLowerCase().replace(/\s+/g, '').replace(/qa/gi, '') + '.com';
      contactEmail = `qa.${firstName.toLowerCase()}.${lastName.toLowerCase()}@${domain}`;
    } else {
      contactEmail = `qa.${contactEmail}`;
    }
...
When('I create 10 real-world Opportunities from Excel data', async function (this: AutomationWorld) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const baseAccountData: any = {
      Name: `QA Opportunity Account ${i + 1} (${accountType})`,
      Type: accountType,
      Account_Status__c: 'Prospect',
      Phone: getPhoneNumber(excelRow, country),
      BillingCountry: country,
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const baseOppData: any = {
      Name: `QA ${oppNames[i % oppNames.length]} - ${companyName}`,
      StageName: stage,
      Type: type,
      Amount: amount,
...
function getCompanyName(excelRow: any, index: number): string {
  ...
  if (lastName && lastName !== 'NULL') {
    return `QA ${lastName} Insurance Group ${index + 1}`;
  }
  ...
  return `QA ${companyNames[index % companyNames.length]}`;
}
