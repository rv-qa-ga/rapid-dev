# ══════════════════════════════════════════════════════════════════════════════
# JIRA: SF-736 — Salesforce Mastered Account (same coverage as legacy ST-191 UI)
...
@integration @mulesoft @salesforce @dynamics @SF-736 @entity_account @entity_party @sf_to_d365 @salesforce_mastered
Feature: SF-736 - Salesforce Mastered Account Integration
  As a Product Owner
  ...
  @SF-736 @SF-736-UI-001 @smoke @positive @integration @sf_to_d365
  Scenario: Create Salesforce Mastered Account and sync to Dynamics Party
    Given I have a test Account name "Salesforce Mastered Account SF-736-001"
    And I subscribe to Account Platform Events
    When I create an Account in Salesforce with:
      | Name | Salesforce Mastered Account SF-736-001 |
      | Type | Customer |
  ...
  @SF-736 @SF-736-UI-002 @positive @integration @sf_to_d365
  Scenario: Update Salesforce Mastered Account and sync to Dynamics Party
    ...
    When I update the Account in Salesforce with:
      | Name | Updated Salesforce Mastered Account SF-736 |
      | BillingStreet | 789 Updated Avenue SF-736 |
    Then an Account Platform Event is published
    And I wait 10 seconds for MuleSoft processing
    Then I retrieve the Party from Dynamics using Party MasterId
    And the Party billing street should be "789 Updated Avenue SF-736"
    Given I unsubscribe from Account Platform Events
  ...
  @SF-736 @SF-736-UI-003 @positive @integration @sf_to_d365
  Scenario: Change Salesforce Mastered Account status and sync to Dynamics
    Given I have a test Account name "UI Status Change SF-736"
    And I subscribe to Account Platform Events
    When I create an Account in Salesforce with:
      | Name | UI Status Change SF-736 |
      | Type | Customer |
      | Account_Status__c | Prospect |
    Then the Account should be created with a SalesforceID
  ...
  @SF-736 @SF-736-UI-005 @positive @integration @sf_to_d365 @onboarding
  Scenario: Create Member Account with Onboarding status and sync to Dynamics Party
    Given I have a test Account name "Member Account Onboarding SF-736"
    And I subscribe to Account Platform Events
    When I create an Account in Salesforce with:
      | Name | Member Account Onboarding SF-736 |
      | Type | Member |
      | Account_Status__c | Prospect |
    Then the Account should be created with a SalesforceID
    And the Account should have PTY_Code__c auto-generated
  ...
  @SF-736 @SF-736-UI-004 @positive @integration @sf_to_d365 @delete
  Scenario: Delete Salesforce Mastered Account and verify Dynamics Party handling
    Given I have a test Account name "Account Delete SF-736"
    And I subscribe to Account Platform Events
    When I create an Account in Salesforce with:
      | Name | Account Delete SF-736 |
      | Type | Customer |
      | Account_Status__c | Prospect |
    Then the Account should be created with a SalesforceID
    When I update the Account Status to "Active" to trigger integration
  ...
  @SF-736 @SF-736-UI-006 @negative @integration @sf_to_d365
  Scenario: Create Account with Draft status should not sync to Dynamics
    Given I have a test Account name "Account Draft Status SF-736"
    When I create an Account in Salesforce with:
      | Name | Account Draft Status SF-736 |
      | Type | Customer |
  ...
  @SF-736 @SF-736-UI-007 @positive @integration @sf_to_d365 @timeliness
  Scenario: Account changes reach Dynamics within 3 minutes - Create
    Given I have a test Account name "Account Timeliness Create SF-736"
    When I create an Account in Salesforce with:
      | Name | Account Timeliness Create SF-736 |
      | Type | Customer |
      | Account_Status__c | Prospect |
    Then the Account should be created with a SalesforceID
    When I update the Account Status to "Active" to trigger integration
  ...
  @SF-736 @SF-736-UI-010 @positive @integration @sf_to_d365 @timeliness
  Scenario: Account changes reach Dynamics within 3 minutes - Update
    Given I have a synced Customer Account in Salesforce for integration tests
    And I have captured the Account SalesforceID and Party MasterId
    When I update the Account in Salesforce with:
      | Name | Account Timeliness Update SF-736 |
      | Phone | 555-888-9999 |
    Then the Dynamics RDM record reflects the change within 3 minutes
    And all mapped fields should match between Salesforce Account and Dynamics Party

  @SF-736 @SF-736-UI-011 @positive @integration @sf_to_d365 @timeliness
  Scenario: Account changes reach Dynamics within 3 minutes - Status Change
    Given I have a test Account name "UI Timeliness Status SF-736"
    And I subscribe to Account Platform Events
    When I create an Account in Salesforce with:
      | Name | UI Timeliness Status SF-736 |
      | Type | Customer |
      | Account_Status__c | Prospect |
    Then the Account should be created with a SalesforceID
    And the Account has Dataverse_ID__c as null
  ...
  @SF-736 @SF-736-UI-008 @positive @integration @sf_to_d365 @null_handling @required_fields
  Scenario: Optional Salesforce fields left empty must be sent as NULL to avoid required-field failures
    Given I have a test Account name "Account Null Fields SF-736"
    When I create an Account in Salesforce with:
      | Name | Account Null Fields SF-736 |
      | Type | Customer |
      | Account_Status__c | Prospect |
      | Phone |  |
      | Email |  |
  ...
  @SF-736 @SF-736-UI-009 @positive @integration @sf_to_d365 @field_mapping @data_integrity
  Scenario: Verify all Account to Party field level requirements match exactly per Excel mapping
    Given I have a test Account name "Account Full Field Mapping SF-736"
    When I create an Account in Salesforce with:
      | Name | Account Full Field Mapping SF-736 |
      | Type | Customer |
      | Account_Status__c | Prospect |
      | Phone | 555-111-2222 |
