/**
 * One-off: create Jira Bug for SF-594/595 CMDT auditability (AC7) and link to parent stories.
 * Run: cross-env ENV=qamerge npx ts-node scripts/create-sf594-595-auditability-bug.ts
 */
...
const SUMMARY =
  '[qamerge] SF-594 / SF-595: Dataverse_Mapping__mdt row-level auditability (when/who/value/domain) not available';

const DESCRIPTION_PLAIN = fs.readFileSync(
  path.join(__dirname, '../docs/bugs/SF-594-595-cmdt-auditability-not-implemented.md'),
  'utf8'
);
...
function plainToAdf(markdown: string): { type: string; version: number; content: unknown[] } {
  ...
  return {
    type: 'doc',
    version: 1,
    content: content.length ? content : [{ type: 'paragraph', content: [{ type: 'text', text: SUMMARY }] }],
  };
}
