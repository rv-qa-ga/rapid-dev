#!/usr/bin/env node
/**
 * Run SF-1081 on qamerge, snapshot isolated Cucumber JSON, upload to Zephyr Sprint-105-QA.
...
const destJson = path.join(root, 'reports/json/sf1081-cucumber-report.json');
...
const testExit = run('test:sf1081:qamerge');
if (fs.existsSync(srcJson)) {
  fs.mkdirSync(path.dirname(destJson), { recursive: true });
  fs.copyFileSync(srcJson, destJson);
}
const zephyrExit = run('zephyr:UploadResult:SF1081:Sprint105');
