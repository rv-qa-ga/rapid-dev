#!/usr/bin/env ts-node
/**
 * Validate Opportunity_Readiness__c Finance section labels (DD stage).
 *
 * Same approach as Compliance: Salesforce describe API on Opportunity_Readiness__c,
 * then check expected Finance section field labels exist. Generates HTML report.
 *
 * Expected fields can be updated in FINANCE_EXPECTED_LABELS (from DD Tasks spec or stakeholder).
 *
 * Usage:
 *   ENV=qa npx ts-node scripts/validate-opportunity-readiness-finance-labels.ts
 *   SF_USE_QA_MRD_FOR_API=true npm run validate:OpportunityReadinessFinance
 */
...
/** Finance section: expected field labels (from DD UI / stakeholder). Add variants if org uses different wording. */
const FINANCE_EXPECTED_LABELS: Array<{ expectedLabel: string; labelVariants?: string[] }> = [
  { expectedLabel: 'Confirm Credit Score Review Completed?' },
  { expectedLabel: 'Parental Company Guarantee Received', labelVariants: ['Parental Company Guarantee Received?', 'Confirm Parental Company Guarantee Received?'] },
  { expectedLabel: 'Financial Accounts Review Completed' },
  { expectedLabel: 'Credit Score' },
  // Add more from Finance DD Tasks when available
];
...
function runSectionChecks(fields: SfField[]): SectionCheckResult[] {
  return FINANCE_EXPECTED_LABELS.map(({ expectedLabel, labelVariants }) => {
...
<head>
  ...
  <title>Opportunity Readiness — Finance Section</title>
  <style>
...
<body>
  <h1>Opportunity_Readiness__c — Finance Section (DD Stage)</h1>
  ...
  <div class="summary ${failCount === 0 ? 'pass' : 'fail'}">
    ...
    ${failCount === 0 ? 'All expected Finance section fields present.' : 'Some fields missing or label mismatch.'}
  </div>
  <table>
    <thead>
      <tr>
...
async function main(): Promise<void> {
  ...
  console.log('═══════════════════════════════════════════════════════════════════════════');
  console.log('  Opportunity_Readiness__c — Finance Section Label Validation');
  console.log('═══════════════════════════════════════════════════════════════════════════\n');
  ...
  console.log('📋 Running Finance section checks...');
  ...
  const htmlPath = path.join(reportDir, `Opportunity-Readiness-Finance-Labels-${timestamp}.html`);
